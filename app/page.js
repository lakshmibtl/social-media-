...
export default function MainMasterSocialFrontend() {
    ...
    async function loadDatabasePhotoPosts() {
      try {
        ...
        const apiPosts = filteredApiPosts.map((item, idx) => {
          ...
          // ✅ Resolve the REAL uploaded image from Drupal (no demo image)
          let postImg = null;
          if (item.type === 'post--photo') {
            ...
            if (fileId && filesMap[fileId]) {
              postImg = filesMap[fileId];
            }
          }
  ...
  const handlePublishPost = async (e) => {
    ...
    const postImg = newPostImage.trim() || null;
    ...
    savePostsState([{
      ...
      content: newPostText, image: postImg, userReaction: null,
    ...
    const savedFile = newPostFile;
    setNewPostText(''); setNewPostImage(''); setNewPostFile(null); setShowPhotoInput(false);
      ...
      if (savedFile) {
        const fileRes = await fetch(`${API_URL}/jsonapi/post/photo/field_post_image`, {
          ...
          headers: {
            ...
            'Content-Disposition': `file; filename="${encodeURIComponent(savedFile.name)}"`,
            'X-CSRF-Token': token
          },
          body: savedFile
        });
  ...
  const handleMainFeedFileUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (ev) => { setNewPostImage(ev.target.result); showToast(`Image "${file.name}" attached!`); };
        ...
        <div style={S.centerCol}>
              ...
              <div style={S.photoPanel}>
                <div style={{ display: 'flex', gap: '12px', marginBottom: '10px', alignItems: 'center' }}>
                  <label style={S.fileUploadBtn}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><FolderUp size={14} /> Select File from Computer</span>
                    <input type="file" accept="image/*" onChange={handleMainFeedFileUpload} style={{ display: 'none' }} />
                  </label>
                ...
                {newPostImage && <img src={newPostImage} alt="Preview" style={{ maxHeight: '140px', borderRadius: '8px', objectFit: 'cover', marginTop: '8px' }} />}
              </div>
            )}
          ...
          {posts.map((post) => (
            ...
            >
              ...
              {post.image && (
                <div style={{ ...S.media, position: 'relative', cursor: 'pointer' }}>
                  <img src={post.image} alt="Media" style={S.mediaImg} />
